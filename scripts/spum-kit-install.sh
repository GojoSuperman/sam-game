...
#   scripts/ · src/            파이프라인 9개 파일
#   package.json               scene-map / studio-login / studio-open / studio-pull
#   .gitignore                 out/ 등
#
...
for f in make-scene-map scene-to-map studio-login studio-open studio-pull; do
  cp "$SRC/scripts/$f.mjs" "$DEST/scripts/"
done
for f in studio-browser studio-scene studio-backup png; do
  cp "$SRC/src/$f.mjs" "$DEST/src/"
done
cp "$SRC/scripts/spum-kit-install.sh" "$DEST/scripts/"
echo "  ✓ 스크립트    scripts/ (6) · src/ (4)"
